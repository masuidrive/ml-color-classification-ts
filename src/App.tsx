...
type LineProps = { x1: number; y1: number; x2: number; y2: number; color: string };
const Line = ({ x1, y1, x2, y2, color }: LineProps) => (
  <line x1={x1} y1={y1} x2={x2} y2={y2} style={{ stroke: color, strokeWidth: 1 }} />
);

type RectProps = { x: number; y: number; width: number; height: number; fill: string; borderColor?: string };
const Rect = ({ x, y, width, height, fill, borderColor }: RectProps) => (
  <rect x={x} y={y} width={width} height={height} style={{ fill, stroke: borderColor, strokeWidth: 1 }}>
...
type CircleProps = { x: number; y: number; radius: number; fill: string; borderColor?: string };
const Circle = ({ x, y, fill, radius, borderColor }: CircleProps) => (
  <circle cx={x} cy={y} r={radius} style={{ fill, stroke: borderColor }} />
);

const times = (n: number, callbackfn: (index: number) => unknown) =>
...
function interpolate(x: number) {
  ...
  const f = x * 255 - a;
  return [
    turbo_colormap_data[a][0] + (turbo_colormap_data[b][0] - turbo_colormap_data[a][0]) * f,
