...
import { Text, Line, Rect, Layer, Stage } from "react-konva";
...
const times = (n:number, callbackfn: (index: number) => unknown) => [...Array(n)].map((_:any, index:number) => callbackfn(index))
...
function interpolate(x: number) {
  x = Math.max(-1.0, Math.min(1.0, x)) / 2 + 0.5
...
function weightColor(val: number | undefined) {
  if (val === undefined) {
    return 'rgba(0,0,0,0)';
  }
  else {
    const [r, g, b] = interpolate(val * 5);
...
const ColorMatrix2D: FC<ColorMatrix2DProps> = ({ matrix, cellSize = 16, rotate = false }) => {
  if (rotate) {
    return <Stage width={matrix.length * cellSize} height={matrix[0].length * cellSize}>
      <Layer>
        {matrix.map((row, x) => row.map((val, y) =>
          <Rect key={`${y}-${x}`} fill={weightColor(val)} x={x * cellSize} y={y * cellSize} width={cellSize} height={cellSize} />
        ))}
      </Layer>
    </Stage>;
    ...
    return <Stage width={matrix[0].length * cellSize} height={matrix.length * cellSize}>
      <Layer>
...
const ColorMatrix1D: FC<ColorMatrix1DProps> = ({ vector, cellSize = 16, direction = "vertical" }) => {
  if (direction == "vertical") {
    return <Stage width={cellSize} height={vector.length * cellSize}>
      <Layer>
        {vector.map((val, y) =>
          <Rect key={`${y}`} fill={weightColor(val)} x={0} y={y * cellSize} width={cellSize} height={cellSize} />
        )}
      </Layer>
    </Stage>;
  }
  else {
    return <Stage width={vector.length * cellSize} height={cellSize}>
      <Layer>
        {vector.map((val, x) =>
          <Rect key={`${x}`} fill={weightColor(val)} x={x * cellSize} y={0} width={cellSize} height={cellSize} />
        )}
      </Layer>
    </Stage>;
  }
...
export default function App() {
  ...
  const form = <div>
    <div className="grid grid-rows-3 grid-flow-col gap-2 grid-cols-[48px_2em_1fr]">
      ...
      <div><Range value={colorR} min={0} max={255} onChange={(e) => setColorR(parseInt(e.target.value, 10))} size="xs" className="range-red" /></div>
      <div><Range value={colorG} min={0} max={255} onChange={(e) => setColorG(parseInt(e.target.value, 10))} size="xs" className="range-green" /></div>
      <div><Range value={colorB} min={0} max={255} onChange={(e) => setColorB(parseInt(e.target.value, 10))} size="xs" className="range-blue" /></div>
    </div>
  ...
  const cellPosX = (x:number, size:number) => (x * 3) * size;
  const cellPosY = (y:number, size:number, height:number, maxHeight:number) => ((y + (maxHeight - height) / 2) * 2) * size;

  type CellProps = { x: number, y: number, size:number, height:number, maxHeight: number, color:string, border?:string };
  const Cell: FC<CellProps> = ({ x, y, size, height, maxHeight, color, border}) => 
    <Rect fill={color} x={(x * 3) * size} y={((y + (maxHeight - height) / 2) * 2) * size} width={size} height={size} stroke={border ?? '#aaaaaa'} strokeWidth={border ? 1 : 0}/>
    ...
    return <div title="Home">
      ...
      <Stage width={500} height={500}>
        <Layer>
          ...
          {rgb.map((val, i) =>
            <Rect
              x={cellPosX(0, cellSize)}
              y={cellPosY(i, cellSize, input_layer, max_layer_height)}
              width={cellSize}
              height={cellSize}
              stroke="#eeeeee"
              fill={`rgb(${i !== 0 ? 255-val : 255},${i !== 1 ? 255-val : 255},${i !== 2 ? 255-val : 255})`}
              key={`${i}`}
            />
          )}
          {["red", "green", "blue"].map((color, i2) => times(hidden_layer, (i1) =>
              <Line
                x={cellSize / 2}
                y={cellSize / 2}
                points={[
                  cellPosX(0, cellSize), cellPosY(i2, cellSize, input_layer, max_layer_height),
                  cellPosX(i2 + 1, cellSize), cellPosY(i1, cellSize, hidden_layer, max_layer_height),
                ]}
                stroke={color}
                lineCap="round"
                key={`${i1}-${i2}`}
          ...
          {/** 隠れ層1 weights */}
          {times(input_layer, (i2) => times(hidden_layer, (i1) =>
            <Rect
              key={`${i1}-${i2}`}
              x={cellPosX(i2 + 1, cellSize)}
              y={cellPosY(i1, cellSize, hidden_layer, max_layer_height)}
              width={cellSize}
              height={cellSize}
              fill={`#eeeeee`}
              stroke="#aaaaaa"
          ...
          {times(input_layer-1, (i2) => times(hidden_layer, (i1) =>
            <Text
              x={cellPosX(i2 + 1.5, cellSize)}
              y={cellPosY(i1, cellSize, hidden_layer, max_layer_height)}
              width={cellSize}
              height={cellSize}
              text="+"
          ...
          {/** 隠れ層1 bias */}
          {times(hidden_layer, (i1) => <>
            <Text
              x={cellSize * 4 + (input_layer * 2) * cellSize + cellSize*1.5}
              y={((i1 + (output_layer - hidden_layer)/2) * 2) * cellSize + 100+1}
              width={cellSize}
              height={cellSize}
              fontSize={cellSize}
              text="→"
            ...
            />
            <Rect
              x={cellSize * 4 + (input_layer * 2) * cellSize + cellSize*3}
              y={((i1 + (output_layer - hidden_layer)/2) * 2) * cellSize + 100+1}
              width={cellSize}
              height={cellSize}
              strokeWidth={1}
